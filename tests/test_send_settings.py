...
def test_settings_exposes_persistent_default_send_destination(tmp_path):
    ...
    combos = [box for box in dialog.findChildren(QComboBox) if box.objectName() == "defaultSendRoute"]
    ...
    assert [(combo.itemData(i), combo.itemText(i)) for i in range(1, 1 + len(SUPPORTED_TARGETS))] == list(
        SUPPORTED_TARGETS
    )
    assert combo.itemData(combo.count() - 1) == "clipboard"
    dialog.close()
